...
class Pss {
  ...
  */
  ...
  encode(message, encodedMessageBits) {
    // Calculate emLen
    const encodedMessageBytes = Math.ceil(encodedMessageBits / 8); // Step 2. mHash = Hash(M)

    const messageHash = crypto.createHash(this.hashAlgorithm).update(message).digest(); // Step 3. If emLen < hLen + sLen + 2, output "encoding error" and stop.
    ...
    const salt = crypto.randomBytes(this.saltLength); // Step 5. M' = (0x)00 00 00 00 00 00 00 00 || mHash || salt
    // Step 6. H = Hash(M')

    const hash = crypto.createHash(this.hashAlgorithm).update(Buffer.alloc(8).fill(0)).update(messageHash).update(salt).digest(); // Step 7. Generate an octet string PS consisting of emLen - sLen - hLen - 2
    //         zero octets.
    // Step 8. Let DB = PS || 0x01 || salt

    const dataBlock = Buffer.concat([Buffer.alloc(encodedMessageBytes - this.saltLength - this.hashLength - 2).fill(0), Buffer.from([1]), salt]); // Step 9. Let dbMask = MGF(H, emLen - hLen - 1)

    const mgf1 = new Mgf1({
      hashAlgorithm: this.hashAlgorithm
    });
    const dataBlockMask = mgf1.generate(hash, encodedMessageBytes - this.hashLength - 1); // Step 10. Let maskedDB = DB \xor dbMask

    const maskedDataBlock = xor(dataBlock, dataBlockMask); // Step 11. Set the leftmost 8emLen - emBits bits of the leftmost octet in
    //          maskedDB to zero.
  ...
  verify(message, encodedMessage, encodedMessageBits) {
    ...
    const messageHash = crypto.createHash(this.hashAlgorithm).update(message).digest(); // Step 3. If emLen < hLen + sLen + 2, output "inconsistent" and stop.
    ...
    const hash = encodedMessage.slice(dataBlockLength, dataBlockLength + this.hashLength); // Step 6. If the leftmost 8emLen - emBits bits of the leftmost octet in
    ...
    const dataBlockMask = mgf1.generate(hash, encodedMessageBytes - this.hashLength - 1); // Step 8. Let DB = maskedDB \xor dbMask.

    const dataBlock = xor(maskedDataBlock, dataBlockMask); // Step 9. Set the leftmost 8emLen - emBits bits of the leftmost octet in DB
    //         to zero.
    ...
    const salt = dataBlock.slice(dataBlock.length - this.saltLength); // Step 12. Let M' = (0x)00 00 00 00 00 00 00 00 || mHash || salt
    // Step 13. Let H' = Hash(M'), an octet string of length hLen.

    const reconstructedHash = crypto.createHash(this.hashAlgorithm).update(Buffer.alloc(8).fill(0)).update(messageHash).update(salt).digest(); // Step 14. If H = H', output "consistent." Otherwise, output "inconsistent."
