...
class Rsa {
  ...
  sign(privateKey, message) {
    ...
    const modulusBitLength = this.getModulusBitLength(modulus); // Pad message using PSS

    const encodedMessage = this.pss.encode(message, modulusBitLength - 1); // OpenSSL expects the message buffer to be the same length (in bytes) as
    // the modulus.

    const paddedMessage = encodedMessage.length < modulus.length ? Buffer.concat([Rsa.ZERO_BYTE, encodedMessage]) : encodedMessage; // Sign
  ...
  verify(modulus, message, signature) {
    ...
    const encodedMessage = modulusBitLength % 8 === 1 ? paddedMessage.slice(1) : paddedMessage; // Verify message padding
