...
const Asn1Condition = require('../schemas/condition').Condition; // Regex for validating conditions
//
// This is a generic, future-proof version of the crypto-condition regular
// expression.


const CONDITION_REGEX = /^ni:\/\/\/sha-256;([a-zA-Z0-9_-]{0,86})\?(.+)$/; // This is a stricter version based on limitations of the current
...
class Condition {
  ...
  getAsn1Json() {
    ...
    if (ConditionClass.TYPE_CATEGORY === 'compound') {
      ...
      const subtypeIds = Array.from(this.getSubtypes()).map(TypeRegistry.findByName).map(x => x.typeId); // Allocate a large enough buffer for the subtypes bitarray
  ...
  validate() {
    ...
    TypeRegistry.findByTypeId(this.getTypeId()); // Bitmask can have at most 32 bits with current implementation
...
Condition.MAX_SAFE_SUBTYPES = 0xffffffff; // Feature suites supported by this implementation

Condition.SUPPORTED_SUBTYPES = 0x3f; // Max fulfillment size supported by this implementation

Condition.MAX_COST = 2097152; // Expose regular expressions
