...
const PrefixFingerprintContents = asn1.define('PrefixFingerprintContents', function () {
  this.seq().obj(this.key('prefix').implicit(0).octstr(), this.key('maxMessageLength').implicit(1).int(), this.key('subcondition').explicit(2).use(Condition));
});
const ThresholdFingerprintContents = asn1.define('ThresholdFingerprintContents', function () {
  this.seq().obj(this.key('threshold').implicit(0).int(), this.key('subconditions').implicit(1).setof(Condition));
});
