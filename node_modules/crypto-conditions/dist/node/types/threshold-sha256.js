...
class ThresholdSha256 extends BaseSha256 {
  ...
  getSubtypes() {
    const typeSets = this.subconditions.map(x => Array.from(x.body.getSubtypes()).concat(x.body.getTypeName()));
    const subtypes = new Set(Array.prototype.concat.apply([], typeSets)); // Never include our own type as a subtype. The reason is that we already
  ...
  getFingerprintContents() {
    return Asn1ThresholdFingerprintContents.encode({
      ...
      subconditions: this.subconditions.map(x => x.body instanceof Condition ? x.body : x.body.getCondition()).sort(ThresholdSha256.compareConditions).map(x => x.getAsn1Json())
    });
  }
  /**
  ...
  calculateCost() {
    ...
    const subconditions = this.subconditions.map(this.constructor.getSubconditionCost);
    const worstCaseFulfillmentsCost = this.constructor.calculateWorstCaseLength(this.threshold, subconditions);
  ...
  static calculateWorstCaseLength(threshold, subconditionCosts) {
    ...
    return subconditionCosts.sort((a, b) => a - b).slice(-threshold).reduce((total, size) => total + size, 0);
  }
  ...
  getAsn1JsonPayload() {
    const fulfillments = this.subconditions.filter(x => x.type === FULFILLMENT).sort((a, b) => a.body.getCondition().getCost() - b.body.getCondition().getCost());
    ...
    const minimalFulfillments = fulfillments.slice(0, this.threshold);
    const remainingConditions = conditions.map(x => x.body).concat(fulfillments.slice(this.threshold).map(x => x.body.getCondition()));
    return {
      subfulfillments: minimalFulfillments.map(x => x.body).sort(ThresholdSha256.compareConditions).map(x => x.getAsn1Json()),
  ...
  validate(message) {
    const fulfillments = this.subconditions.filter(cond => cond.type === FULFILLMENT); // Number of fulfilled conditions must meet the threshold
